    ...
    methods
        ...
        function inputSignal(obj, signal)
            ...
            if size(obj.nodes, 1) < 3
                obj.addNode(signal);
                obj.signalnum2(1,obj.signalNum)=obj.signalNum;
                return;
            end
        ...
        function show(obj, varargin)
            ...
            title(strcat('nodes:', num2str(size(obj.nodes,1)), ' edges:', num2str(sum(sum((obj.adjacencyMat > 0)))), ' ġWinTime:', num2str(sum(obj.winningTimes))));
        ...
        function rgbs = getRgbVectors(obj, num)
            ...
            for i = 0:num-1
                rgbs(i+1,:) = obj.str2numArry(dec2base(i,base,3));
            end
            if base == 3
                rgbs = rgbs/2;
            end
        ...
        function deleteOldEdges(obj, winnerIndex)
            ...
            deletedNodeIndexes = [];
            for i = 1: length(indexes)
                if obj.adjacencyMat(indexes(i), winnerIndex) > obj.maxEdgeAge + 1 % 1 expresses that there is an edge.
